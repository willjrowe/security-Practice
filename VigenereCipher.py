...
from constants import testKeyword

class VigenereCipher(Cipher):
    def __init__(self):
        name = "Vigenere Cipher"
        explanation = "A bit more complicated"
        additionalInputs = ["keyword"]
    ...
    def encrypt(self,*args):
        if len(args) > 1:
            ...
            plaintext = args[0]
            keyword = random.choice(testKeyword)
        else:
            plaintext = input("Enter plaintext: ")
            keyword = input("Enter keyword (a string): ")
